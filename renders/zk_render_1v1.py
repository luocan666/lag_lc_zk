...
from envs.JSBSim.envs.zk import ZKSingleCombatEnv
...
env = ZKSingleCombatEnv(scenario_name, 2)
env.seed(0)
args = Args()
...
while True:
    ...
    if dones.all():
        print(infos)
        break
    bloods = [env.agents[agent_id].get("LifeCurrent") for agent_id in env.agents.keys()]
